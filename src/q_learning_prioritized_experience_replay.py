...
class ReplayBuffer:
    ...
    def sample(self, weights):
        ...
        probas = list(weights.values())
        return random.choices(elements, weights=probas)
...
class QLearningPrioExpReplayAgent(BaseAgent):
    ...
    def learn(self):
        transition = self.memory.sample(self.weights)
...
def train(env, state_dim, action_dim, num_episodes, max_steps_per_episode, target_score):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps_per_episode):
            ...
            agent.weights[(state, action, reward, next_state, done)] = abs(reward + agent.gamma * np.max(agent.q_table[next_state, :]) - agent.q_table[state, action])
            agent.learn()
